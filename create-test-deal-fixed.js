...
async function createTestDeal() {
  ...
  try {
    ...
    // 3. Create a WFM project for the deal (using correct column name)
    const { data: wfmProject, error: wfmError } = await supabase
      .from('wfm_projects')
      .insert({
        project_type_id: projectType.id,
        workflow_id: projectType.default_workflow_id,
        current_step_id: initialStep.id,  // Fixed: was current_wfm_step_id
        name: 'Test Deal for Conversion',
        description: 'Test deal created for conversion testing'
      })
      .select()
      .single();
