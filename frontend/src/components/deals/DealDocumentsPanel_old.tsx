...
const DealDocumentsPanel: React.FC<DealDocumentsPanelProps> = ({ dealId, dealName = 'Deal' }) => {
  ...
  // Deal-centric state
  const [dealFolderInfo, setDealFolderInfo] = useState<DealFolderInfo | null>(null);
  const [dealFolderFiles, setDealFolderFiles] = useState<DealFolderFile[]>([]);
  const [selectedSubfolder, setSelectedSubfolder] = useState<string | null>(null);
  const [attachedDocuments, setAttachedDocuments] = useState<DealDocument[]>([]);
  const [viewMode, setViewMode] = useState<'deal-folder' | 'import-from-drive'>('deal-folder');
